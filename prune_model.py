...
def main():
    ...
    parser.add_argument("--skip_bottom", type=int, default=8, help="Number of bottom MLP layers to skip (protects ID-to-token mapping)")
    parser.add_argument("--skip_top", type=int, default=2, help="Number of top MLP layers to skip (protects task projection)")
    ...
    # Protection Mask: Prevent pruning bottom/top bottleneck layers
    num_layers = len(mlp_scores)
    mask = np.ones(num_layers, dtype=bool)
    if args_cli.skip_bottom > 0:
        mask[:args_cli.skip_bottom] = False
        print(f"Protecting first {args_cli.skip_bottom} layers.")
    if args_cli.skip_top > 0:
        mask[-args_cli.skip_top:] = False
        print(f"Protecting last {args_cli.skip_top} layers.")
    
    # Set scores of protected layers to infinity so they are never picked
    masked_scores = mlp_scores.copy()
    masked_scores[~mask] = np.inf
    
    # Get indices of the k smallest scores from the allowed range
    prune_indices = np.argsort(masked_scores)[:args_cli.top_k].tolist()
    # Filter out layers that still have inf score (in case top_k > allowed layers)
    prune_indices = [idx for idx in prune_indices if masked_scores[idx] != np.inf]
    
    print(f"Identified MLP layers to prune (with skip logic): {prune_indices}")
