...
def main_worker(local_rank, args):
    ...
    if args.distributed:
        ...
        dist.init_process_group(
            ...
            init_method="env://",  # env:// is recommended when using torchrun
        )
    ...
    if 'Qwen' in args.backbone:
        ...
        elif args.prompt_selection == 1:
            text = 'In heterogeneous user interactions, extract information useful for recommendation in the target domain a a a and exclude irrelevant information, then compress the filtered information into the last token:'
        elif args.prompt_selection == 2:
            text = 'x'
        ...
        # special_tokens_dict = {'additional_special_tokens': ['<|user|>', '<|slow|>', '<|fast|>','<|trigger|>','<|thought|>']}
